...
function loadPackageManifest(logger, packageJsonPath) {
    const packageJson = JSON.parse(fs_2.readFile(packageJsonPath));
...
function globPackages(workspacePath, packageJsonDir) {
    return glob.sync(path_1.resolve(packageJsonDir, workspacePath, "package.json"), {
        ignore: path_1.resolve(packageJsonDir, workspacePath, "node_modules"),
    });
}
/**
...
function expandPackages(logger, packageJsonDir, workspaces) {
    ...
    return array_1.flatMap(workspaces, (workspace) => {
        const globbedPackageJsonPaths = globPackages(workspace, packageJsonDir);
        return array_1.flatMap(globbedPackageJsonPaths, (packageJsonPath) => {
            ...
            if (packagePaths === undefined) {
                ...
                return [path_1.dirname(packageJsonPath)];
            }
            // This is a workspace root package, recurse
            return expandPackages(logger, path_1.dirname(packageJsonPath), packagePaths);
        });
    });
}
...
function getTsSourceFromJsSource(logger, jsPath) {
    const contents = fs_2.readFile(jsPath);
    ...
    if (sourceMapURL.startsWith("data:application/json;base64,")) {
        ...
        resolvedSourceMapURL = path_1.resolve(jsPath, "..", sourceMapURL);
        sourceMap = JSON.parse(fs_2.readFile(resolvedSourceMapURL));
    }
    ...
    if (sourceRoot !== undefined) {
        ...
        source = path_1.join(sourceRoot, source);
    }
    const sourcePath = path_1.resolve(resolvedSourceMapURL, "..", source);
    return sourcePath;
}
...
function getTsEntryPointForPackage(logger, packageJsonPath, packageJson) {
    ...
    let entryPointPath = path_1.resolve(packageJsonPath, "..", packageMain);
    ...
    try {
        ...
        if (/\.tsx?$/.test(entryPointPath) && fs_1.existsSync(entryPointPath)) {
        ...
        if (e.code !== "MODULE_NOT_FOUND") {
            ...
            entryPointPath = path_1.resolve(packageJsonPath, "..", packageTypes !== null && packageTypes !== void 0 ? packageTypes : packageMain);
            if (/\.tsx?$/.test(entryPointPath) && fs_1.existsSync(entryPointPath)) {
                return entryPointPath;
            }
            else {
