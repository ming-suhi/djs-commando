...
class TypeDocReader {
    ...
    readFile(file, container, logger, seen) {
        ...
        if ("extends" in data) {
            ...
            for (const extendedFile of extended) {
                ...
                this.readFile(path_1.resolve(path_1.dirname(file), extendedFile), container, logger, seen);
            }
            delete data["extends"];
        }
        for (const [key, val] of Object.entries(data)) {
            try {
                container.setValue(key, val, path_1.resolve(path_1.dirname(file)));
            }
            catch (error) {
    ...
    findTypedocFile(path) {
        ...
        return [
            path,
            path_1.join(path, "typedoc.json"),
            path_1.join(path, "typedoc.js"),
