...
let ImplementsPlugin = class ImplementsPlugin extends components_1.ConverterComponent {
    ...
    analyzeClass(context, classReflection, interfaceReflection) {
        ...
        interfaceReflection.children.forEach((interfaceMember) => {
            ...
            if (interfaceMember.kindOf(index_1.ReflectionKind.FunctionOrMethod) &&
                ...
                for (const [clsSig, intSig] of array_1.zip(classMember.signatures, interfaceMember.signatures)) {
                    if (clsSig.implementationOf) {
                        clsSig.implementationOf = new index_2.ReferenceType(clsSig.implementationOf.name, intSig, context.project);
                    }
                    reflections_1.copyComment(clsSig, intSig);
                }
            }
        });
    }
    analyzeInheritance(context, reflection) {
        var _a, _b, _c, _d, _e;
        const extendedTypes = array_1.filterMap((_a = reflection.extendedTypes) !== null && _a !== void 0 ? _a : [], (type) => {
            ...
            for (const parentMember of (_b = parent.reflection.children) !== null && _b !== void 0 ? _b : []) {
                ...
                if (child) {
                    const key = child.overwrites
                        ? "overwrites"
                        : "inheritedFrom";
                    for (const [childSig, parentSig] of array_1.zip((_d = child.signatures) !== null && _d !== void 0 ? _d : [], (_e = parentMember.signatures) !== null && _e !== void 0 ? _e : [])) {
                        childSig[key] = new index_2.ReferenceType(`${parent.name}.${parentMember.name}`, parentSig, context.project);
                        reflections_1.copyComment(childSig, parentSig);
                    }
    ...
    tryResolve(context, reflection) {
        var _a, _b, _c, _d;
        const requirements = array_1.filterMap([
            ...((_a = reflection.implementedTypes) !== null && _a !== void 0 ? _a : []),
            ...((_b = reflection.extendedTypes) !== null && _b !== void 0 ? _b : []),
        ], (type) => {
            return type instanceof index_2.ReferenceType ? type.reflection : void 0;
        });
...
function constructorInheritance(context, reflection, childDecl, constructorDecl) {
    ...
    (_b = reflection[key]) !== null && _b !== void 0 ? _b : (reflection[key] = index_2.ReferenceType.createBrokenReference(name, context.project));
    for (const sig of (_c = reflection.signatures) !== null && _c !== void 0 ? _c : []) {
        (_d = sig[key]) !== null && _d !== void 0 ? _d : (sig[key] = index_2.ReferenceType.createBrokenReference(name, context.project));
    }
}
...
function createLink(context, reflection, clause, expr, symbol, isOverwrite) {
    ...
    function link(target) {
        ...
        if (!target)
            return;
        if (clause.token === ts.SyntaxKind.ImplementsKeyword) {
            (_a = target.implementationOf) !== null && _a !== void 0 ? _a : (target.implementationOf = index_2.ReferenceType.createBrokenReference(name, project));
            return;
        }
        if (isOverwrite) {
            (_b = target.inheritedFrom) !== null && _b !== void 0 ? _b : (target.inheritedFrom = index_2.ReferenceType.createBrokenReference(name, project));
        }
        else {
            (_c = target.overwrites) !== null && _c !== void 0 ? _c : (target.overwrites = index_2.ReferenceType.createBrokenReference(name, project));
        }
    }
}
