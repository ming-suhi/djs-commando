...
let CommentPlugin = CommentPlugin_1 = class CommentPlugin extends components_1.ConverterComponent {
    ...
    onCreateTypeParameter(_context, reflection, node) {
        if (node && ts.isJSDocTemplateTag(node.parent)) {
            const comment = comment_1.getJsDocCommentText(node.parent.comment);
            if (comment) {
    ...
    onDeclaration(context, reflection, node) {
        ...
        let rawComment = exportNode && comment_1.getRawComment(exportNode, this.application.logger);
        rawComment !== null && rawComment !== void 0 ? rawComment : (rawComment = node && comment_1.getRawComment(node, this.application.logger));
        if (!rawComment) {
            return;
        }
    ...
    onBeginResolve(context) {
        ...
        const [allRemoved, someRemoved] = utils_1.partition(utils_1.filterMap(hidden, (reflection) => {
            var _a;
        ...
        someRemoved.forEach((reflection) => {
            reflection.sources = utils_1.unique(reflection.signatures.reduce((c, s) => c.concat(s.sources || []), []));
        });
    }
    /**
